 ...
 *
 */
public class WechatAppPayOrder extends BasePayOrder implements PayOrderInterface {
 
	Logger logger = Logger.getLogger(WechatAppPayOrder.class);
	...
	public Map<String,String> doPay(PayParamsBean payParams)  throws BaseException {
        ...
        Map<String,String> secSignMap = parsePreResult(payParams.getBaseOrder().getBaseordersn()
        		...
        		,payParams.getImei(),payParams.getIp());
        String secSign = null;
		try {
	 ...
	 * @return
	 */
	private String preRequestDisposed(String baseOrdersn,BigDecimal sellprice
			,String tmei,String ip){
		//根据userid获取openid
		if("".equals(tmei)){
			tmei = null;
		}
		...
		ScanPayReqData sp = new ScanPayReqData(appid,mch_id,
				null, body, null, baseOrdersn,
				sellprice.multiply(new BigDecimal("100")).intValue(),
	 ...
	 * @return
	 */
	private Map<String,String> parsePreResult(String baseOrderSn,BigDecimal sellprice,String tmei,String ip){
		String returnMsg = preRequestDisposed(baseOrderSn,sellprice,tmei,ip);
